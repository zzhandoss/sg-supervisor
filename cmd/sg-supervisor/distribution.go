package main

import (
	"context"
	"errors"
	"flag"
	"fmt"
	"os"

	"sg-supervisor/internal/app"
)

func runBuildDistribution(ctx context.Context, args []string) error {
	fs := flag.NewFlagSet("build-distribution", flag.ContinueOnError)
	root := fs.String("root", ".", "supervisor root")
	platform := fs.String("platform", "", "target platform: windows or linux")
	binaryPath := fs.String("binary", "", "supervisor binary path")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if *platform == "" {
		return errors.New("platform is required")
	}

	supervisor, err := app.New(*root)
	if err != nil {
		return err
	}
	report, err := supervisor.BuildDistribution(ctx, *platform, *binaryPath)
	if err != nil {
		return err
	}
	if report.ArtifactPath != "" {
		fmt.Println(report.ArtifactPath)
	} else {
		fmt.Println(report.OutputDir)
	}
	for _, warning := range report.Warnings {
		fmt.Fprintln(os.Stderr, warning)
	}
	return nil
}
